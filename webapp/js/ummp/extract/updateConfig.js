...
function commit(){
	...
	array.push(param);
	$.ajax({
		url:'../extract/saveUpdateGetDataSqlConfigInfo',
		data:'s='+ encodeURIComponent($.toJSON(array)),
		type:'post',
		cache:false,
...
function onClickParamRow(index,row){
	...
	//reloadCombox('paramTypeId',type); 	  //此方式不能赋值
	$("#paramTypeId").combobox("setValue",type);
	reloadCombox('paramValueTypeId',valueType);
	//reloadCombox('paramSubTypeId',subType); //此方式不能赋值
