...
@Service
public class SHIAC04RDeal implements ReturnDealService {
	...
	public void dealRiskInfo(Map<String,String> map){
		...
		try {
			...
			record.setPolicySequenceNo(map.get("CLAIM_SEQUENCE_NO"));//团险理赔编码
			record.setCustomerNo(map.get("CUSTOMER_NO"));//客户序号
			...
			record.setRiskType("2");//1-标识个险 2-标识团险
	...
	public void dealTrace(Map<String, String> map){
		...
		tDefTrace.setStandby2(map.get("CLAIM_SEQUENCE_NO"));
		tDefTrace.setStandby3(map.get("CUSTOMER_NO"));
		tDefTrace.setStandby4(map.get("CUTOMER_SEQUENCE_NO"));
		tDefTrace.setBussType("LP01");
		String stateFlag="";
		if(map.get("RESPONSE_CODE").equals("1")){
			 stateFlag = "6";
		}else{
	...
	public void dealReturn(Map<String, String> map){
		...
		tDefReturn.setStandby2(map.get("CUSTOMER_NO"));
		tDefReturn.setStandby3(map.get("CUTOMER_SEQUENCE_NO"));
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println(map.get("CUSTOMER_NO"));
