...
/**
 * 个单核保信息同步上传 返回处理，调用核保信息同步上传返回处理。<br>
 * 因为是根据接口的BUSS_TYPE构造返回处理类，所以为区分团险个险将接口升级为SHIAU0102 则要有其相对应的返回处理
 * @author Darker.Wang
 * @date 2016-12-6 下午4:38:34
 * @company SINOSOFT
 */
@Service
public class SHIAU0102RDeal implements ReturnDealService{
	@Autowired
	...
	public boolean dealReturn(SubmitReturn sr, ParamPOJO pojo) {
		...
		System.out.println("SHIAU0102 返回处理，调用SHIAU01的返回处理： "+result);
		return result;
	}
}
