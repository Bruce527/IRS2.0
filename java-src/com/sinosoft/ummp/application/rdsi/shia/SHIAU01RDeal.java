...
@Service
public class SHIAU01RDeal implements ReturnDealService {
	...
	Logger logger = Logger.getLogger(SHIAU01RDeal.class);
	...
	public boolean dealReturn(SubmitReturn sr, ParamPOJO pojo) {
		...
		BussModel model = sr.getBussModel();//获得业务模型
		...
		try {
			...
			for( Iterator<?> a = root.elementIterator();a.hasNext();){
				...
				if(itemEle.getText() != null){
					map.put(itemEle.getName(), itemEle.getText());
				}
...
//		往treace/return表处理更新数据
	...
	public void dealRiskInfo(Map<String,String> map){
		...
		try {
			...
			record.setUnderwritSequenceNo(map.get("UNDERWRIT_SEQUENCE_NO"));//核保确认编码
	...
	public void dealTrace(Map<String, String> map){
		...
		tDefTrace.setStandby2(map.get("UNDERWRIT_SEQUENCE_NO"));
		tDefTrace.setStandby3(map.get("CUTOMER_SEQUENCE_NO"));
		tDefTrace.setBussType("HB");
		String stateFlag="";
		if(map.get("RESPONSE_CODE").equals("1")){
			 stateFlag = "2";
		}else{
	...
	public void dealReturn(Map<String, String> map){
		...
		tDefReturn.setQueryNo(map.get("UNDERWRIT_SEQUENCE_NO"));
		tDefReturn.setStandby2(map.get("CUTOMER_SEQUENCE_NO"));
...
//					System.out.println("key:"+itemEle.getName());//head body
				}
				for(Iterator<?> b = itemEle.elementIterator();b.hasNext();){
					Element itemEle1 = (Element) b.next();
					if(itemEle1.getText() != null){
						map.put(itemEle1.getName(), itemEle1.getTextTrim());
//						System.out.println("key:"+name+"value:"+value);//1 2 3 4
					}
					for(Iterator<?> c = itemEle1.elementIterator();c.hasNext();){
						Element itemEle2 = (Element)c.next();
						if(itemEle2.getText() != null){
							map.put(itemEle2.getName(), itemEle2.getTextTrim());
//							System.out.println("key:"+itemEle2.getName()+"  .... value:"+itemEle2.getText());//0005286987426
						}
						for(Iterator<?> d = itemEle2.elementIterator();d.hasNext();){
							Element itemEle3 = (Element)d.next();
							if(itemEle3.getText() != null){
								map.put(itemEle3.getName(), itemEle3.getTextTrim());
//								System.out.println("key:"+itemEle3.getName());
							}
							for(Iterator<?> e = itemEle3.elementIterator();e.hasNext();){
								Element itemEle4 = (Element)e.next();
								if(itemEle4.getText() != null){
									map.put(itemEle4.getName(), itemEle4.getTextTrim());
//									System.out.println("key:"+itemEle4.getName()+"  value:"+itemEle4.getText());//110 120
//									list.add(map);
								}
								for(Iterator<?> x = itemEle4.elementIterator();x.hasNext();){
									Element itemEle5 = (Element)x.next();
									if(itemEle5.getText() != null){
										map.put(itemEle5.getName(), itemEle5.getTextTrim());
//										System.out.println(itemEle5.getName());
									}
									for(Iterator<?> l = itemEle5.elementIterator();l.hasNext();){
										Element itemEle6 = (Element)l.next();
										if(itemEle6.getText() != null){
											map.put(itemEle6.getName(), itemEle6.getTextTrim());
//											System.out.println("key:"+itemEle6.getName()+"  value:"+itemEle6.getText());
										}
									}
								}
							}
			...
			System.out.println(map.get("UNDERWRIT_SEQUENCE_NO"));
