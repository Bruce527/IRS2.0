...
public class MethodInvokingJobDetailFactoryBean implements FactoryBean, BeanNameAware, InitializingBean,ApplicationContextAware {
	...
	public void afterPropertiesSet() throws Exception {
		try {
			mLogger.debug("- start");
			mLogger.debug("- Creating JobDetail " + beanName);
			...
			mLogger.debug("- Registering JobListener names with JobDetail object " + beanName);
			if (this.jobListenerNames != null) {
				for (int i = 0; i < this.jobListenerNames.length; i++) {
					this.jobDetail.addJobListener(this.jobListenerNames[i]);
				}
			}
			mLogger.info("- Created JobDetail: " + jobDetail + "; targetClass: " + targetClass + "; targetObject: " + targetObject + "; targetMethod: " + targetMethod + "; staticMethod: " + staticMethod
					+ "; arguments: " + arguments + ";");
		} finally {
			mLogger.debug("- end");
		}
	...
	public static class MethodInvokingJob implements Job {
		...
		public void execute(JobExecutionContext context) throws JobExecutionException {
			//TODO 执行批处理
			//获取 QRTZ_TRIGER_LOG 批处理日志表主键
			Long id = System.currentTimeMillis();                    
			//声明批处理日志接口引用为null
			QuartzLogService quartzLogService = applicationContext.getBean(QuartzLogService.class);   
			logger.debug("- QuartzLogService "+quartzLogService);
			
			JobDetail tJobDetial = null;      						 //声明JobDetail对象引用为null
			Trigger tTrigger = null;			  					 //声明Trigger对象引用为null	
			boolean batchVerify  = true;                            //批处理校验是否通过标志
			
			
			
			tJobDetial = context.getJobDetail();                  //获取JobDetail实例对象
			tTrigger = context.getTrigger();					     //获取Trigger实例对象
			logger.info("- "+DateUtils.getCurrentDateTime()
					+" Execute Job: "
					+" JobName = "+tTrigger.getJobName()
					+" TriggerName = "+tTrigger.getName()
					+" StartDate = "+tTrigger.getStartTime()
					+" EndDate = "+tTrigger.getEndTime()
					+" JobClass = "+tJobDetial.getJobClass());
			JobDataMap jobDataMap = tJobDetial.getJobDataMap();
//			JobDataMap jobDataMap = context.getMergedJobDataMap();
			String[] keys = jobDataMap.getKeys();
			System.out.println("- context.jobDataMap: "+keys);
			logger.info("- 准备前置校验开始: ");
			String insertMsg = "执行就绪";
			try {
				batchVerify = quartzLogService.batchVerify(tJobDetial, tTrigger);//计划执行【批处理前置校验】-->true：校验通过，false：校验不通过
				insertMsg = "前置校验完毕： "+batchVerify;
			}catch(Exception e){
				insertMsg = "前置校验异常";
			}
			System.out.println("- 前置校验结果: "+insertMsg);
			//设置起始时间，以便日志调用插入
			tTrigger.setStartTime(new Date());
			quartzLogService.insertLog(id, tJobDetial, tTrigger,1,insertMsg);          //校验通过插入批处理日志操作
			try {
				if (batchVerify) {
					// targetClass 获取加载
					String targetClass = jobDataMap.getString("targetClass");
					...
					// targetObject targetMethod staticMethod arguments 等加载
					...
					String staticMethod = jobDataMap.getString("staticMethod");
					...
					logger.info("- Invoking: "
							+" targetClass: "+targetClassClass
							+" targetObject: "+targetObject
							+" targetMethod: "+targetMethod
							+"\nmethodInvoker : "+ methodInvoker.getPreparedMethod()
									.toGenericString());
					methodInvoker.invoke();
	
					// 设置结束时间，以便日志调用插入
					tTrigger.setEndTime(new Date());// 调用目标对象方法
					// 更新批处理日志
					quartzLogService.uploadLog(id, tJobDetial, tTrigger, 1);
				...
				/*else{
					...
					tTrigger.setEndTime(new Date());//调用目标对象方法
					//更新批处理日志
					quartzLogService.uploadLog(id, tJobDetial, tTrigger,0,"前置校验未通过"); 
				}*/
			}catch (Exception e) {
				e.printStackTrace();
				System.out.println("- EXECUTE: jobId = "+id+" jobName = "+tTrigger.getJobName()+" \n- EXCEPTION :"+e.getMessage());
				tTrigger.setEndTime(new Date());//调用目标对象方法
				quartzLogService.uploadLog(id, tJobDetial, tTrigger,-1,"执行异常");                   //执行过程捕捉异常
